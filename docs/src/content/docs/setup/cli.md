...
**`--gh-aw-ref` flag:** Convenience alias for `--action-mode release --action-tag <ref>`. Accepts a branch name, tag, or commit SHA targeting the installed gh-aw source repository. Upstream releases resolve refs in `github/gh-aw`; fork releases built with a stamped source repository resolve refs in that fork, for example `JKamsker/gh-aw`. Branch and tag names are resolved to their full commit SHA at compile time, so the baked-in reference is immutable and reproducible. Useful for E2E-testing workflows compiled against a specific gh-aw revision.
